...
void player::AnimationManager::selectAnimation() const {
    ...
    switch (pPlayer->stateManager.engine.pCurrentState->stateID) {
        case JUMPING : {
            pPlayer->animationManager.engine.set(JUMPING);
            pPlayer->animationManager.engine.animationSet[JUMPING].fps = std::fabs(pPlayer->movement.walkingSpeed.y / pPlayer->movement.speed.y)*24.f;
            break;
        }
        case WALKING : {
            pPlayer->animationManager.engine.set(WALKING);
            pPlayer->animationManager.engine.pCurrentAnimation->fps = std::fabs(pPlayer->physics.velocity.x / pPlayer->movement.walkingSpeed.x) * static_cast<float>(pPlayer->animationManager.engine.pCurrentAnimation->framesPerRow) * 2.f;
            break;
        }
        case RUNNING : {
